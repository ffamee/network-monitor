...
export async function addProbe(
	...
	const validatedFields = ProbeSchema.safeParse({ ...rawData, buildingId });
	...
	try {
		console.log(
			`Add new probe to ${buildingId} with data:`,
			validatedFields.data,
		);
		const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/probe`, {
			method: "POST",
			headers: {
